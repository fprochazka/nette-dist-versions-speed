...
namespace {/**
 * Nette Framework (version 2.1-dev released on 2013-03-28, http://nette.org)
 *
...
IBarPanel{function
getTab();function
getPanel();}}namespace Nette{use
Nette;interface
IFreezable{function
freeze();function
isFrozen();}}namespace Nette\Security{use
Nette;interface
IIdentity{function
getId();function
getRoles();}interface
...
getRoleId();}interface
IResource{function
getResourceId();}}namespace Nette\Reflection{use
Nette;interface
IAnnotation{function
__construct(array$values);}}namespace Nette\Caching{use
Nette;interface
IStorage{function
read($key);function
lock($key);function
...
clean(array$conditions);}}namespace Nette\Localization{use
Nette;interface
ITranslator{function
translate($message,$count=NULL);}}namespace Nette\Forms{use
Nette;interface
IFormRenderer{function
render(Form$form);}}namespace Nette\ComponentModel{use
Nette;interface
IComponent{const
...
getValidationScope();}}namespace Nette\Templating{use
Nette;interface
ITemplate{function
render();}interface
IFileTemplate
...
getFile();}}namespace Nette\Application{use
Nette;interface
IResponse{function
...
constructUrl(Request$appRequest,Nette\Http\Url$refUrl);}}namespace Nette\Application\UI{use
Nette;interface
...
isControlInvalid();}}namespace Nette\Application{use
Nette;interface
IPresenter{function
run(Request$request);}interface
IPresenterFactory{function
getPresenterClass(&$name);function
createPresenter($name);}}namespace Nette\Mail{use
Nette;interface
IMailer{function
send(Message$mail);}}namespace Nette\Http{use
Nette;interface
IResponse{const
...
remove($id);function
clean($maxlifetime);}}namespace Nette\Config{use
Nette;interface
IAdapter{function
load($file);function
dump(array$data);}}namespace Nette\Latte{use
Nette;interface
IMacro{function
initialize();function
finalize();function
nodeOpened(MacroNode$node);function
nodeClosed(MacroNode$node);}}namespace Nette\Database{use
Nette;interface
IReflection{const
...
isSupported($item);}}namespace Nette{use
Nette;class
...
instanceof\Closure){return'{closure}';}elseif(is_string($this->cb)&&$this->cb[0]==="\0"){return'{lambda}';}else{is_callable($this->cb,TRUE,$textual);return$textual;}}}final
class
Environment{const
...
isset($params[$key])?$params[$key]:$default;}else{return$params;}}}abstract
class
FreezableObject
extends
Object
...
class
Framework{const
NAME='Nette Framework',VERSION='2.1-dev',REVISION='7bde9b1 released on 2013-03-28';public
static$iAmUsingBadHost=FALSE;final
function
__construct(){throw
new
StaticClassException;}}final
class
ObjectMixin{private
...
FALSE;}elseif(isset($l[''][$class])){return$l[''][$class];}$cl=$class;do{if(isset($l[$cl])){return$l[''][$class]=$l[$cl];}}while(($cl=strtolower(get_parent_class($cl)))!=='');foreach(class_implements($class)as$cl){$cl=strtolower($cl);if(isset($l[$cl])){return$l[''][$class]=$l[$cl];}}return$l[''][$class]=FALSE;}}class
Image
extends
Object{const
SHRINK_ONLY=1;const
...
modifyClone($modify=''){$dolly=clone$this;return$modify?$dolly->modify($modify):$dolly;}}}namespace Nette\Security{use
Nette;class
Identity
extends
...
function
getAuthorizator(){return$this->authorizator?:$this->context->getByType('Nette\Security\IAuthorizator');}}}namespace Nette\Security\Diagnostics{use
Nette;class
UserPanel
extends
Nette\Object
...
self::ALLOW;}}private
function&getRules($resource,$role,$create=FALSE){$null=NULL;if($resource===self::ALL){$visitor=&$this->rules['allResources'];}else{if(!isset($this->rules['byResource'][$resource])){if(!$create){return$null;}$this->rules['byResource'][$resource]=array();}$visitor=&$this->rules['byResource'][$resource];}if($role===self::ALL){if(!isset($visitor['allRoles'])){if(!$create){return$null;}$visitor['allRoles']['byPrivilege']=array();}return$visitor['allRoles'];}if(!isset($visitor['byRole'][$role])){if(!$create){return$null;}$visitor['byRole'][$role]['byPrivilege']=array();}return$visitor['byRole'][$role];}}}namespace Nette\Reflection{use
Nette;use
...
__isset($name){return
ObjectMixin::has($this,$name);}function
__unset($name){ObjectMixin::remove($this,$name);}}use
Nette\Utils\Strings;/**
...
__isset($name){return
ObjectMixin::has($this,$name);}function
__unset($name){ObjectMixin::remove($this,$name);}}}namespace Nette\Caching{use
Nette;class
OutputHelper
extends
...
new
Nette\InvalidStateException('Output cache has already been saved.');}$this->cache->save($this->key,ob_get_flush(),(array)$dependencies+(array)$this->dependencies);$this->cache=NULL;}}class
Cache
extends
Nette\Object
...
Nette\Reflection\ClassType::from($class)->getAnnotation('serializationVersion')===$value;}}}namespace Nette\Caching\Storages{use
Nette;use
...
parent::getCacheFile(substr_replace($key,trim(strtr($this->hint,'\\/@','.._'),'.').'-',strpos($key,Nette\Caching\Cache::NAMESPACE_SEPARATOR)+1,0)).'.php';}}class
MemoryStorage
extends
Nette\Object
implements
...
write($key,$data,array$dependencies){}function
remove($key){}function
clean(array$conditions){}}}namespace Nette\Loaders{use
Nette;use
Nette\Caching\Cache;class
RobotLoader
extends
...
function
getKey(){return
array($this->ignoreDirs,array_keys($this->filters),$this->scanDirs);}}}namespace Nette\Forms{use
Nette;final
class
Rules
extends
...
CONDITION=1;const
VALIDATOR=2;const
FILTER=3;public$control;public$operation;public$arg;public$type;public$isNegative=FALSE;public$message;public$subRules;}}namespace Nette\ComponentModel{use
Nette;abstract
class
Component
extends
...
function
getSession(){return
Nette\Environment::getSession();}}class
ControlGroup
extends
...
instanceof
Http\FileUpload&&$file->isImage();}}use
...
getControl(){$control=parent::getControl();foreach($this->getRules()as$rule){if($rule->isNegative||$rule->type!==Nette\Forms\Rule::VALIDATOR){}elseif($rule->operation===Nette\Forms\Form::RANGE&&$control->type!=='text'){list($control->min,$control->max)=$rule->arg;}elseif($rule->operation===Nette\Forms\Form::PATTERN){$control->pattern=$rule->arg;}}if($control->type!=='password'){$control->value=$this->getValue()===''?$this->translate($this->emptyValue):$this->value;}return$control;}}class
SelectBox
extends
BaseControl{const
VALID=':selectBoxValid';private$items=array();protected$allowed=array();private$prompt=FALSE;private$useKeys=TRUE;function
__construct($label=NULL,array$items=NULL,$size=NULL){parent::__construct($label);$this->control->setName('select');$this->control->size=$size>1?(int)$size:NULL;if($items!==NULL){$this->setItems($items);}$this->addRule($this->validateSelectBoxValid,Nette\Forms\Rules::$defaultMessages[self::VALID]);}function
getValue(){return
...
instanceof
Nette\Utils\Html){$dest->add((string)$value2->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$value2=$this->translate((string)$value2);$dest->add((string)$option->value($key2)->selected(isset($selected[$key2]))->setText($value2));}}}return$control;}static
function
validateSelectBoxValid(Nette\Forms\IControl$control){return$control->prompt!==FALSE||$control->getValue()!==NULL;}}class
HiddenField
extends
...
getLabel($caption=NULL){$label=parent::getLabel($caption);$label->for=NULL;return$label;}}}namespace Nette\Forms\Rendering{use
Nette;use
...
function
getValue($name){$name=explode(' ',$name);$data=&$this->wrappers[$name[0]][$name[1]];return$data;}}}namespace Nette\Templating{use
Nette;use
Nette\Caching;class
Template
extends
...
instanceof
Caching\Storages\PhpFileStorage){Nette\Utils\LimitedScope::load($cached['file'],$this->getParameters());}else{Nette\Utils\LimitedScope::evaluate($compiled,$this->getParameters());}}}class
FilterException
extends
...
setSourceFile($file){$this->sourceFile=(string)$file;$this->message=rtrim($this->message,'.')." in ".str_replace(dirname(dirname($file)),'...',$file).($this->sourceLine?":$this->sourceLine":'');}}use
Nette\Utils\Strings;use
Nette\Forms\Form;use
...
Nette\Utils\Json::encode($this->payload);}}class
TextResponse
extends
Nette\Object
implements
...
instanceof
Nette\Templating\ITemplate){$this->source->render();}else{echo$this->source;}}}class
FileResponse
extends
Nette\Object
implements
...
send(Nette\Http\IRequest$httpRequest,Nette\Http\IResponse$httpResponse){}}use
Nette\Http;class
RedirectResponse
extends
Nette\Object
...
send(Http\IRequest$httpRequest,Http\IResponse$httpResponse){$httpResponse->redirect($this->url,$this->code);}}}namespace Nette\Application\UI{use
Nette;abstract
class
PresenterComponent
extends
...
getSnippetId($name=NULL){return'snippet-'.$this->getUniqueId().'-'.$name;}}use
...
getUser(){return$this->user;}}class
...
new
BadSignalException("Missing handler for signal '$signal' in $class.");}}}}namespace Nette\Application{use
Nette;class
AbortException
extends\Exception{}class
...
extends
BadRequestException{protected$defaultCode=403;}}namespace Nette\Application\UI{use
Nette;class
BadSignalException
extends
Nette\Application\BadRequestException{protected$defaultCode=403;}class
Multiplier
extends
...
function
createComponent($name){return$this->factory->invoke($name,$this);}}class
Link
extends
...
__toString(){try{return(string)$this->component->link($this->destination,$this->params);}catch(\Exception$e){trigger_error("Exception in ".__METHOD__."(): {$e->getMessage()} in {$e->getFile()}:{$e->getLine()}",E_USER_ERROR);}}}use
...
FALSE;}}return
TRUE;}}}namespace NetteModule{use
Nette;use
Nette\Application;use
Nette\Application\Responses;use
Nette\Http;class
...
new
Application\BadRequestException($message,$code);}function
getRequest(){return$this->request;}}use
...
return
new
Application\Responses\TextResponse(ob_get_clean());}}}namespace Nette\Application\Diagnostics{use
Nette;use
...
restoreRequest($key){trigger_error(__METHOD__.'() is deprecated; use $presenter->restoreRequest() instead.',E_USER_DEPRECATED);return$this->presenter->restoreRequest($key);}}}namespace Nette\Application\Routers{use
Nette;use
...
new
Nette\InvalidArgumentException("Style '$style' doesn't exist.");}static::$styles[$style][$key]=$value;}}class
CliRouter
extends
Nette\Object
implements
...
NULL;}function
getDefaults(){return$this->defaults;}}}namespace Nette\Application{use
Nette;final
class
Request
extends
...
formatPresenterFile($presenter){$path='/'.str_replace(':','Module/',$presenter);return$this->baseDir.substr_replace($path,'/presenters',strrpos($path,'/'),0).'Presenter.php';}}}namespace Nette\Mail{use
Nette;use
Nette\Utils\Strings;class
MimePart
extends
...
new
Nette\InvalidStateException("Unable to send email: $error[message].");}}}}namespace Nette\Http{use
Nette;final
class
Response
extends
...
deleteCookie($name,$path=NULL,$domain=NULL,$secure=NULL){$this->setCookie($name,FALSE,0,$path,$domain,$secure);}}use
Nette\Utils\Strings;class
RequestFactory
extends
Nette\Object{const
...
new
Request($url,$query,$post,$files,$cookies,$headers,isset($_SERVER['REQUEST_METHOD'])?$_SERVER['REQUEST_METHOD']:NULL,isset($_SERVER['REMOTE_ADDR'])?$_SERVER['REMOTE_ADDR']:NULL,isset($_SERVER['REMOTE_HOST'])?$_SERVER['REMOTE_HOST']:NULL);}}use
...
getContents(){return$this->isOk()?file_get_contents($this->tmpName):NULL;}}}namespace Nette\Iterators{use
Nette;class
Mapper
...
__unset($name){Nette\ObjectMixin::remove($this,$name);}}}namespace Nette\DI\Diagnostics{use
Nette;use
...
extends
Nette\InvalidStateException{}use
...
generateClass(){throw
new
Nette\DeprecatedException(__METHOD__.'() is deprecated; use generateClasses()[0] instead.');}}final
class
Helpers{static
function
...
new
Nette\InvalidStateException("Property $property has not @var annotation.");}elseif(!class_exists($type)&&!interface_exists($type)){if($type[0]!=='\\'){$type=$class->getNamespaceName().'\\'.$type;}if(!class_exists($type)&&!interface_exists($type)){throw
new
Nette\InvalidStateException("Please use a fully qualified name of class/interface in @var annotation at $property property. Class '$type' cannot be found.");}}$res[$property->getName()]=$type;}return$res;}}}namespace Nette\Config{use
Nette;use
...
afterCompile(Nette\PhpGenerator\ClassType$class){}}}namespace Nette\Config\Extensions{use
Nette;use
...
extends
Nette\Config\CompilerExtension{function
loadConfiguration(){foreach($this->getConfig()as$name=>$class){$this->compiler->addExtension($name,new$class);}}}use
...
new
Nette\NotSupportedException;}function&__get($name){trigger_error("Service accessing via nette->$name is deprecated, use 'nette.$name'.",E_USER_DEPRECATED);$service=$this->container->getService("nette.$name");return$service;}}class
PhpExtension
extends
Nette\Config\CompilerExtension{function
...
Nette\DI\Statement($v->value,self::filterArguments($v->attributes));}}return$args;}}class
Loader
extends
Nette\Object{const
...
function
getSection(array$data,$key,$file){Validators::assertField($data,$key,'array|null',"section '%' in file '$file'");$item=$data[$key];if($parent=Helpers::takeParent($item)){$item=Helpers::merge($item,$this->getSection($data,$parent,$file));}return$item;}}use
Nette\Caching\Cache;class
Configurator
extends
Nette\Object{const
...
function
detectProductionMode($list=NULL){trigger_error(__METHOD__.'() is deprecated; use !detectDebugMode() instead.',E_USER_DEPRECATED);return!static::detectDebugMode($list);}}}namespace Nette\Config\Adapters{use
Nette;use
Nette\Config\Helpers;use
...
dump(array$data){return"<?php // generated by Nette \nreturn ".Nette\PhpGenerator\Helpers::dump($data).';';}}class
IniAdapter
extends
Nette\Object
implements
...
new
Nette\InvalidArgumentException("The '$prefix$key' item must be scalar or array, ".gettype($val)." given.");}}}}}namespace Nette\Config{use
Nette;class
Helpers{const
...
is_array($data)&&isset($data[self::EXTENDS_KEY])&&$data[self::EXTENDS_KEY]!==self::OVERWRITE;}}}namespace Nette\ComponentModel{use
Nette;class
...
Compiler::CONTENT_TEXT:return$s;default:return"\$template->escape($s)";}}}use
Nette\Utils\Strings;class
Parser
extends
Nette\Object{const
...
setArgs($args){$this->args=(string)$args;$this->tokenizer->tokenize($this->args);}}}namespace Nette\Utils{use
Nette;class
Tokenizer
extends
...
fetchWord(){$word=$this->fetchUntil(self::T_WHITESPACE,',');$this->fetch(',');$this->fetchAll(self::T_WHITESPACE,self::T_COMMENT);return$word;}}}namespace Nette\Latte\Macros{use
Nette;use
Nette\Latte;class
CacheMacro
extends
...
instanceof
Nette\ComponentModel\IContainer){$queue[]=$child;}}}while($queue);}}}}namespace Nette\Latte{use
Nette;class
Token
extends
Nette\Object{const
...
getCompiler(){return$this->compiler;}}}namespace Nette\Database\Reflection{use
Nette;class
...
function
getColumnFromTable($name){if($this->table!=='%s'&&preg_match('(^'.str_replace('%s','(.*)',preg_quote($this->table)).'\z)',$name,$match)){return$match[1];}return$name;}}use
...
Connection
extends
Nette\Object{private$dsn;private$driver;private$preprocessor;private$selectionFactory;private$pdo;public$onQuery;function
__construct($dsn,$user=NULL,$password=NULL,array$options=NULL,$driverClass=NULL){$this->pdo=$pdo=new
PDO($this->dsn=$dsn,$user,$password,$options);$pdo->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);$pdo->setAttribute(PDO::ATTR_STATEMENT_CLASS,array('Nette\Database\Statement',array($this)));$driverClass=$driverClass?:'Nette\Database\Drivers\\'.ucfirst(str_replace('sql','Sql',$pdo->getAttribute(PDO::ATTR_DRIVER_NAME))).'Driver';$this->driver=new$driverClass($this,(array)$options);$this->preprocessor=new
SqlPreprocessor($this);}function
getDsn(){return$this->dsn;}function
getPdo(){return$this->pdo;}function
getSupplementalDriver(){return$this->driver;}function
beginTransaction(){return$this->pdo->beginTransaction();}function
commit(){return$this->pdo->commit();}function
rollBack(){return$this->pdo->rollBack();}function
getInsertId($name=NULL){return$this->pdo->lastInsertId($name);}function
quote($string,$type=PDO::PARAM_STR){return$this->pdo->quote($string,$type);}function
...
queryArgs($statement,array$params){if($params){list($statement,$params)=$this->preprocessor->process($statement,$params);}return$this->pdo->prepare($statement)->execute($params);}function
...
lastInsertId($name=NULL){trigger_error(__METHOD__.'() is deprecated; use getInsertId() instead.',E_USER_DEPRECATED);return$this->getInsertId($name);}}}namespace Nette\Database\Diagnostics{use
Nette;use
...
instanceof
SqlLiteral){return$value->__toString();}else{$this->remaining[]=$value;return'?';}}}class
Helpers{static
...
Nette\Database\Diagnostics\ConnectionPanel($connection);$panel->explain=$explain;Nette\Diagnostics\Debugger::$bar->addPanel($panel);return$panel;}}}namespace Nette\Database\Table{use
Nette;use
...
extends
Nette\Object
implements\Iterator,\ArrayAccess,\Countable{protected$connection;protected$reflection;protected$cache;protected$sqlBuilder;protected$name;protected$primary;protected$primarySequence=FALSE;protected$rows;protected$data;protected$dataRefreshed=FALSE;protected$globalRefCache;protected$refCache;protected$specificCacheKey;protected$aggregation=array();protected$accessedColumns;protected$previousAccessedColumns;protected$observeCache=FALSE;protected$checkReferenced=FALSE;protected$keys=array();function
...
getName(){return$this->name;}function
getPrimary(){if($this->primary===NULL){throw
new\LogicException("Table \"{$this->name}\" does not have a primary key.");}return$this->primary;}function
getPrimarySequence(){if($this->primarySequence===FALSE){$this->primarySequence=NULL;$primary=$this->getPrimary();$driver=$this->connection->getSupplementalDriver();if($driver->isSupported(ISupplementalDriver::SUPPORT_SEQUENCE)){foreach($driver->getColumns($this->name)as$column){if($column['name']===$primary){$this->primarySequence=$column['vendor']['sequence'];break;}}}}return$this->primarySequence;}function
...
getPreviousAccessedColumns(){if($this->cache&&$this->previousAccessedColumns===NULL){$this->accessedColumns=$this->previousAccessedColumns=(array)$this->cache->load($this->getGeneralCacheKey());}return
...
function
emptyResultSet(){$this->rows=NULL;$this->specificCacheKey=NULL;}protected
function
saveCacheState(){if($this->observeCache===$this&&$this->cache&&!$this->sqlBuilder->getSelect()&&$this->accessedColumns!=$this->previousAccessedColumns){$this->cache->save($this->getGeneralCacheKey(),$this->accessedColumns);}}protected
function
getRefTable(&$refPath){return$this;}protected
function
getGeneralCacheKey(){return
md5(serialize(array(__CLASS__,$this->name,$this->sqlBuilder->getConditions())));}protected
function
...
instanceof\Traversable){$data=iterator_to_array($data);}$return=$this->connection->query($this->sqlBuilder->buildInsertQuery(),$data);$this->checkReferenced=TRUE;if(!is_array($data)){return$return->rowCount();}if(!is_array($this->primary)&&!isset($data[$this->primary])&&($id=$this->connection->getInsertId($this->getPrimarySequence()))){$data[$this->primary]=$id;}$row=$this->createRow($data);if($signature=$row->getSignature(FALSE)){$this->rows[$signature]=$row;}return$row;}function
update($data){if($data
...
getReferencedTable($table,$column,$checkReferenced=FALSE){$referenced=&$this->refCache['referenced'][$this->getSpecificCacheKey()]["$table.$column"];if($referenced===NULL||$checkReferenced||$this->checkReferenced){$this->execute();$this->checkReferenced=FALSE;$keys=array();foreach($this->rows
...
function
execute(){if($this->rows!==NULL){$this->observeCache=$this;return;}$hash=$this->getSpecificCacheKey();$accessedColumns=$this->accessedColumns;$referencing=&$this->refCache['referencing'][$this->getGeneralCacheKey()];$this->observeCache=&$referencing['observeCache'];$this->accessedColumns=&$referencing[$hash]['accessed'];$this->specificCacheKey=&$referencing[$hash]['specificCacheKey'];$this->rows=&$referencing[$hash]['rows'];$data=&$referencing[$hash]['data'];if($data===NULL){$this->accessedColumns=$accessedColumns;$limit=$this->sqlBuilder->getLimit();$rows=count($this->refTable->rows);if($limit&&$rows>1){$this->sqlBuilder->setLimit(NULL,NULL);}parent::execute();$this->sqlBuilder->setLimit($limit,NULL);$data=array();$offset=array();$this->accessColumn($this->column);foreach((array)$this->rows
as$key=>$row){$ref=&$data[$row[$this->column]];$skip=&$offset[$row[$this->column]];if($limit===NULL||$rows<=1||(count($ref)<$limit&&$skip>=$this->sqlBuilder->getOffset())){$ref[$key]=$row;}else{unset($this->rows[$key]);}$skip++;unset($ref,$skip);}}$this->observeCache=$this;$this->data=&$data[$this->active];if($this->data===NULL){$this->data=array();}else{foreach($this->data
...
__construct($tableName,Connection$connection,IReflection$reflection){$this->tableName=$tableName;$this->databaseReflection=$reflection;$this->driver=$connection->getSupplementalDriver();$this->driverName=$connection->getPdo()->getAttribute(\PDO::ATTR_DRIVER_NAME);$this->delimitedTable=$this->tryDelimite($tableName);}function
...
FALSE;}$this->conditions[$hash]=$condition;$condition=$this->removeExtraTables($condition);$condition=$this->tryDelimite($condition);$placeholderCount=substr_count($condition,'?');if($placeholderCount>1&&count($args)===2&&is_array($parameters)){$args=$parameters;}else{array_shift($args);}if($placeholderCount===0&&count($args)===1){$condition.=' ?';}elseif($placeholderCount!==count($args)){throw
new
...
as$arg){preg_match('#(?:.*?\?.*?){'.$placeholderNum.'}(((?:&|\||^|~|\+|-|\*|/|%|\(|,|<|>|=|ALL|AND|ANY|BETWEEN|EXISTS|IN|LIKE|OR|NOT|SOME)\s*)?\?)#',$condition,$match,PREG_OFFSET_CAPTURE);$hasOperator=($match[1][0]==='?'&&$match[1][1]===0)?TRUE:!empty($match[2][0]);if($arg===NULL){if($hasOperator){throw
new
...
new
Nette\InvalidArgumentException('Column operator does not accept array argument.');}}else{$match[2][0]='IN ';}if(!$arg){$replace=$match[2][0].'(NULL)';}else{$replace=$match[2][0].'(?)';$this->parameters[]=$arg;}}else{if($hasOperator){$replace=$match[2][0].'?';}else{$replace='= ?';}$this->parameters[]=$arg;}if($replace){$condition=substr_replace($condition,$replace,$match[1][1],strlen($match[1][0]));$replace=NULL;}if($arg!==NULL){$placeholderNum++;}}$this->where[]=$condition;return
TRUE;}function
...
FALSE;}}}namespace Nette\Utils{use
Nette;class
Strings{final
function
__construct(){throw
...
__construct($message,$code=NULL,$pattern=NULL){if(!$message){$message=(isset(self::$messages[$code])?self::$messages[$code]:'Unknown error').($pattern?" (pattern: $pattern)":'');}parent::__construct($message,$code);}}final
class
Arrays{final
function
__construct(){throw
...
function
isList($value){return
is_array($value)&&(!$value||array_keys($value)===range(0,count($value)-1));}}class
Neon
extends
Nette\Object{const
BLOCK=1;private
...
hasChildren(){return
parent::hasChildren(TRUE);}}}}namespace Nette\PhpGenerator{use
Nette;class
Method
extends
...
__call($name,$args){return
Nette\ObjectMixin::callProperty($this,$name,$args);}}use
Nette\Utils\Strings;class
ClassType
extends
...
unserialize('O'.substr(serialize((string)$class),1,-1).substr(serialize($props),1));}}}namespace {Nette\Diagnostics\Debugger::_init();Nette\Utils\SafeStream::register();function
